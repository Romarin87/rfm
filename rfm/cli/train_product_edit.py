"""CLI for R-only top-K product graph-edit prediction."""
...
from rfm.models import ProductEditPredictor, load_pretrained_encoder
...
def main(argv: list[str] | None = None) -> None:
    ...
    train_ds = product_edit.ProductEditDataset(args.train, args.max_train, args)
    valid_ds = product_edit.ProductEditDataset(args.valid, args.max_valid, args, delta_vocab=train_ds.delta_vocab)
    test_ds = product_edit.ProductEditDataset(args.test, args.max_test, args, delta_vocab=train_ds.delta_vocab)
    ...
    train_loader = DataLoader(
        ...
        collate_fn=product_edit.collate,
        num_workers=args.num_workers,
        pin_memory=torch.cuda.is_available(),
    )
    valid_loader = DataLoader(valid_ds, batch_size=args.batch_size, shuffle=False, collate_fn=product_edit.collate, num_workers=args.num_workers, pin_memory=torch.cuda.is_available())
    test_loader = DataLoader(test_ds, batch_size=args.batch_size, shuffle=False, collate_fn=product_edit.collate, num_workers=args.num_workers, pin_memory=torch.cuda.is_available())

    input_schema, pair_raw_dim = product_edit_schema(args)
    model = ProductEditPredictor(
        pair_raw_dim,
        args.hidden_dim,
        args.layers,
        args.dropout,
    ...
    delta_class_weights = product_edit.class_weights(train_ds.delta_vocab, args, device)
    ...
    for epoch in range(args.epochs):
        ...
        train_metrics = product_edit.train_one_epoch(train_model, train_loader, optimizer, device, args, delta_class_weights)
        stop_now = False
        if is_main(rank):
            valid_loss = product_edit.evaluate_loss(model, valid_loader, device, args, delta_class_weights)
            valid_metrics, _ = product_edit.evaluate(model, valid_loader, device, train_ds.delta_vocab, max_prediction_rows=0)
    ...
    if is_main(rank):
        ...
        valid_metrics, valid_rows = product_edit.evaluate(model, valid_loader, device, train_ds.delta_vocab, max_prediction_rows=args.max_prediction_rows)
        test_metrics, test_rows = product_edit.evaluate(model, test_loader, device, train_ds.delta_vocab, max_prediction_rows=args.max_prediction_rows)
        best = {
            "epoch": best_epoch,
            "seed": args.seed,
            "world_size": world,
        ...
        torch.save(
            {
                ...
                "model_class": "ProductEditPredictor",
            },
            output_dir / "model.pt",
        )
        torch.save({"adapter": model.input_adapter.state_dict(), "config": vars(args)}, output_dir / "product_edit_adapter.pt")
        ...
        torch.save({"product_edit_head": model.product_edit_head.state_dict(), "config": vars(args), "delta_vocab": train_ds.delta_vocab}, output_dir / "product_edit_head.pt")
        ...
        write_json(
            output_dir / "data_manifest.json",
            {
                ...
                "geometry_mode": args.geometry_mode,
                "input_schema": input_schema,
                "task": "R-only top-K product graph-edit prediction",
                "input_policy": {
                    "uses_BO_R": True,
        ...
        product_edit.write_summary(output_dir, best)
        append_registry(
            args,
            output_dir,
            stage="product_edit_prediction",
            task="r_only_topk_product_graph_edit",
            model="ProductEditPredictor",
            status="complete",
            notes=f"input_schema={input_schema}; geometry_mode={args.geometry_mode}; top_k={args.top_k}; selection=minimum valid_loss; optimizer={args.optimizer}",
        )
