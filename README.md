...
`train_product_edit` is the R-only top-K product graph-edit task. Its encoder
input is restricted to `Z`, `BO_R`, optional `D_R_irc`, and optional R-side
frozen Suiren features; `BO_P` and `Delta_BO` are labels only.
